...
var protondex$1 = require('./abstract/protondex.js');
var number = require('./base/functions/number.js');
var errors = require('./base/errors.js');
var Precise = require('./base/Precise.js');
var sha256 = require('./static_dependencies/noble-hashes/sha256.js');
var ripemd160 = require('./static_dependencies/noble-hashes/ripemd160.js');
var secp256k1 = require('./static_dependencies/noble-curves/secp256k1.js');
var utils = require('./static_dependencies/noble-curves/abstract/utils.js');

//  ---------------------------------------------------------------------------
//  ---------------------------------------------------------------------------
class protondex extends protondex$1 {
    describe() {
    ...
    async fetchMyTrades(symbol = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        if (params['account'] === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' fetchMyTrades() requires a account argument in params');
        }
        const request = {
    ...
    async fetchClosedOrders(symbol = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        if (params['account'] === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' fetchOrders() requires a account argument in params');
        }
        const request = {
            'account': params['account'],
            'status': 'delete',
    ...
    parseOrder(order, market = undefined) {
        ...
        if (side === '1' && status === 'closed' && symbol !== undefined) {
            amountString = parseFloat(Precise["default"].stringDiv(amountString, this.safeString(order, 'avgPrice'))).toFixed(market.info.bid_token.precision);
            remainingString = parseFloat(Precise["default"].stringDiv(remainingString, this.safeString(order, 'avgPrice'))).toFixed(market.info.bid_token.precision);
        }
            ...
            if (order['quantity_init'] === order['quantity_change']) {
                ...
                if (side === '1' && symbol !== undefined) {
                    ...
                    amountString = parseFloat(Precise["default"].stringDiv(this.safeString(order, 'quantity_init'), this.safeString(order, 'avgPrice'))).toFixed(market.info.bid_token.precision);
                    remainingString = parseFloat(Precise["default"].stringDiv(remainingString, this.safeString(order, 'avgPrice'))).toFixed(market.info.bid_token.precision);
                    filled = parseFloat(Precise["default"].stringDiv(filled, this.safeString(order, 'avgPrice'))).toFixed(market.info.bid_token.precision);
                }
            }
        }
    ...
    async fetchOrderTrades(id, symbol = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        await this.loadMarkets();
        if (params['account'] === undefined) {
            throw new errors.ArgumentsRequired(this.id + ' fetchOrderTrades() requires a account argument in params');
        }
        let market = undefined;
        const request = {
            'account': params['account'],
    ...
    async fetchOHLCV(symbol, timeframe = '1W', since = undefined, limit = undefined, params = {}) {
        /**
         * @method
    ...
    digestSuffixRipemd160(data, suffix) {
        ...
        return ripemd160.ripemd160(d);
    }
    keyToString(keyData, suffix, prefix) {
    ...
    fromElliptic(ellipticSig) {
        ...
        const r = utils.numberToBytesBE(ellipticSig.r, 32);
        const s = utils.numberToBytesBE(ellipticSig.s, 32);
        ...
        const sigData = utils.concatBytes(initParams, r, s);
    ...
    getSignatures(transHex) {
        ...
        const signBuf = utils.concatBytes(utils.hexToBytes(chainID), transHex, initData);
    ...
    async createOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        if (this.secret === undefined) {
            throw new errors.ArgumentsRequired('createOrder call requires a secret to be set');
        }
        const market = this.market(symbol);
        ...
        while (retries > 0) {
            try {
                ...
                const tx = utils.hexToBytes(result['serialized_tx_hex']);
    ...
    async cancelOrder(id, symbol = undefined, params = {}) {
        ...
        await this.loadMarkets();
        if (this.secret === undefined) {
            throw new errors.ArgumentsRequired('cancelOrder call requires a secret to be set');
        }
    ...
    async getorderIds(name, symbol) {
        try {
            ...
            throw new errors.OrderNotFound(' calcelAllOrders() error: order not found');
        }
    }
    async cancelAllOrders(symbol = undefined, params = {}) {
        ...
        if (!orderList.length) {
            throw new errors.OrderNotFound(' calcelAllOrders() error: no orders found');
        }
        const actions = [];
        const auth = { 'actor': accountName, 'permission': 'active' };
